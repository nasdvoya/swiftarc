...
<div class="max-w-4xl mx-auto p-6">
    ...
    <form onsubmit={handleSubmit}>
        <div class="grid grid-cols-2 gap-x-8 gap-y-4 text-left">
            <!-- Left Column -->
            <div>
                <Input name="nome" label="Nome" pattern="^[A-Za-zÀ-ÖØ-öø-ÿ\s]+$"
                    errorMessage="O nome nao pode conter simbolos ou numeros!" placeholder="John Doe"
                    bind:value={formData.nome} />
                <Input name="email" label="Email" pattern="^[^@]+@[^@]+\.[^@]+$" errorMessage="Email inválido!"
                    placeholder="johndoe@example.com" bind:value={formData.email} />
                <Input name="horario" label="Horário" pattern="^(?:[01]\d|2[0-3]):[0-5]\d$"
                    errorMessage="Hora inválida!" placeholder="HH:MM" bind:value={formData.horario} />
            ...
            <div>
                ...
                <Input name="codigo_postal" label="C.P." pattern="^\d{4}-\d{3}$" placeholder="Código Postal"
                    errorMessage="Código Postal inválido!" bind:value={formData.codigo_postal} />
                <Input name="tel_residencial" label="Tel. Residencial" placeholder="Telefone Residencial"
                    errorMessage="Número de telefone inválido!" bind:value={formData.tel_residencial} />
                <Input name="tel_trabalho" label="Tel. Trabalho" placeholder="Telefone Trabalho"
                    errorMessage="Número de telefone inválido!" bind:value={formData.tel_trabalho} />
                <Input name="telemovel" label="Telemóvel" placeholder="Telemóvel"
                    errorMessage="Número de telefone inválido!" bind:value={formData.telemovel} />
            </div>
